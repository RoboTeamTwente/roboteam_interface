...
void InterfaceBallItem::trigger_update(const proto::State &state) {
    ...
    double x = this->scene()->views()[0]->viewport()->width()/2 + (actualScale * state.last_seen_world().ball().pos().x() * 1000) - 5;
    double y = this->scene()->views()[0]->viewport()->height()/2 + (actualScale * state.last_seen_world().ball().pos().y() * 1000) - 5;

    this->setPos(x, y);
}

void InterfaceBallItem::updateScale(double fieldWidth, double fieldHeight) {
    if (fieldWidth == 0 || fieldHeight == 0) this->actualScale = 0;
    ...
    this->actualScale = std::fmin(widthScale, heightScale);
}